  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLTSignalRHostApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(invokeMethodMethodName:connectionId:arguments:completion:)], @"FLTSignalRHostApi api (%@) doesn't respond to @selector(invokeMethodMethodName:connectionId:arguments:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_methodName = GetNullableObjectAtIndex(args, 0);
        NSString *arg_connectionId = GetNullableObjectAtIndex(args, 1);
        NSArray<NSString *> *arg_arguments = GetNullableObjectAtIndex(args, 2);
        [api invokeMethodMethodName:arg_methodName connectionId:arg_connectionId arguments:arg_arguments completion:^(NSString *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    }
