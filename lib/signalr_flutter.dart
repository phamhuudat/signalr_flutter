...
import 'package:signalr_multi_connect/signalr_platform_interface.dart';
...
class SignalR extends SignalrPlatformInterface implements SignalRPlatformApi {
  ...
  SignalR(
    ...
    void Function(ConnectionStatus?, String?)? statusChangeCallback,
  ...
  Future<void> onStatusChange(StatusChangeResult statusChangeResult) async {
    ...
    statusChangeCallback?.call(statusChangeResult.status, connectionId);
  ...
  Future<String> invokeMethod(String methodName, {List<String>? arguments}) async {
    try {
      return await _signalrApi.invokeMethod(methodName, connectionId ?? '', arguments ?? List.empty());
    } catch (e) {
      return Future.error(e);
    }
  }
